/**
 * @file sorting_searching.cpp
 * @brief Implementation of sorting and searching algorithm visualizer
 * 
 * This class visualizes 7 algorithms:
 * - Linear Search: O(n) - Sequentially checks each element
 * - Binary Search: O(log n) - Searches in sorted array by dividing in half
...
#include <QHBoxLayout>        ///< Horizontal layout for arranging widgets side-by-side
#include <QVBoxLayout>        ///< Vertical layout for stacking widgets
#include <QGridLayout>        ///< Grid layout for complex widget arrangements
#include <QPushButton>        ///< Clickable button widgets for user interaction
#include <QLineEdit>          ///< Single-line text input for array values
#include <QSpinBox>           ///< Number input with increment/decrement buttons
#include <QSlider>            ///< Slider control for animation speed adjustment
#include <QLabel>             ///< Text label for displaying information
#include <QFrame>             ///< Container widget with border styling
#include <QScrollArea>        ///< Scrollable container for long content
#include <QSpacerItem>        ///< Flexible spacing in layouts
#include <QStyle>             ///< Qt style system for widget appearance

// --------------------- Qt Core Includes -------------------------
#include <QRandomGenerator>   ///< Generates random numbers for array initialization
#include <QtMath>             ///< Mathematical functions (min, max, etc.)
#include <QStringList>        ///< List of strings for text processing

// --------------------- Standard Library Includes ----------------
#include <algorithm>          ///< STL algorithms (std::sort, std::swap, etc.)
      ...
      m_searchTarget(0)
{
    ...
    // Note: updateAlgorithm is called but may return early if algorithm already matches
    // So we explicitly show target widget for LinearSearch (default)
    if (m_targetWidget) {
        m_targetWidget->setVisible(true);
    }
...
void SortingVisualizer::setupUi()
{
    ...
    m_targetSpinBox = new QSpinBox(targetWidget);
    ...
    // Create a small widget to hold the custom buttons
    auto targetButtonContainer = new QWidget(targetWidget);
    ...
    legendLayout->addWidget(makeLegendItem(tr("Found"), QColor(52, 152, 219), this), 0, Qt::AlignLeft);
    legendLayout->addWidget(makeLegendItem(tr("Pivot"), QColor(155, 89, 182), this), 0, Qt::AlignLeft);
    legendLayout->addWidget(makeLegendItem(tr("Sorted"), QColor(46, 204, 113), this), 0, Qt::AlignLeft);
    legendLayout->addStretch();
    inputLayout->addLayout(legendLayout);
    
    // Initially show target field since LinearSearch is the default algorithm
    if (m_targetWidget) {
        m_targetWidget->setVisible(true);
    }
...
void SortingVisualizer::clearTransientStates()
{
    for (int i = 0; i < static_cast<int>(m_barStates.size()); ++i) {
        // Preserve Found state (blue) - don't clear it
        if (m_barStates[i] == ArrayVisualizer::BarState::Found) {
            continue;
        }
...
void SortingVisualizer::setStatesForIndices(const std::vector<int> &indices, ArrayVisualizer::BarState state)
{
    for (int idx : indices) {
        if (idx < 0 || idx >= static_cast<int>(m_barStates.size()))
            continue;
        // Allow Found state to override sorted flags (for search algorithms)
        if (state != ArrayVisualizer::BarState::Sorted && 
            state != ArrayVisualizer::BarState::Found &&
...
void SortingVisualizer::enqueueLinearSearch(std::vector<int> arr, int target)
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (arr[i] == target) {
            ...
            // Step: Found the target - make it blue
            m_stepQueue.push([this, finalIndex, target]() {
                clearTransientStates();
                setStatesForIndices({finalIndex}, ArrayVisualizer::BarState::Found);
                setStatusMessage(tr("Blue target found: %1 at index %2!").arg(target).arg(finalIndex));
            });
            break;
        }
    }
...
void SortingVisualizer::enqueueBinarySearch(std::vector<int> arr, int target)
{
    ...
    while (left <= right) {
        ...
        m_stepQueue.push([this, currentLeft, currentRight, currentMid, midValue, target]() {
            clearTransientStates();
            // Highlight the search range - mid element uses Comparing state (yellow)
            for (int i = currentLeft; i <= currentRight; ++i) {
                if (i == currentMid) {
                    setStatesForIndices({i}, ArrayVisualizer::BarState::Comparing);
                } else {
...
void SortingVisualizer::enqueueInsertionSort(std::vector<int> arr)
{
    ...
    for (int i = 1; i < n; ++i) {
        ...
        const int keyValue = key;
        m_stepQueue.push([this, insertIndex, keyValue]() {
            clearTransientStates();
            setStatesForIndices({insertIndex}, ArrayVisualizer::BarState::Comparing);
            setStatusMessage(tr("Inserting value %1 at index %2").arg(keyValue).arg(insertIndex));
        });

        while (j >= 0 && arr[j] > key) {
            const int from = j;
            const int to = j + 1;
            const int shiftValue = arr[j];
            arr[j + 1] = arr[j];
            
            // Show comparing before shifting
            m_stepQueue.push([this, from, to, keyValue, shiftValue]() {
                clearTransientStates();
                setStatesForIndices({from, to}, ArrayVisualizer::BarState::Comparing);
                setStatusMessage(tr("Comparing %1 (index %2) with key %3 - shifting needed")
                                     .arg(shiftValue).arg(from).arg(keyValue));
            });
            
            // Shift the element - update the visualization array
            m_stepQueue.push([this, from, to, shiftValue]() {
                clearTransientStates();
                m_currentArray[to] = shiftValue;
                setStatesForIndices({from, to}, ArrayVisualizer::BarState::Swapping);
                setStatusMessage(tr("Shifting %1 from index %2 to %3")
                                     .arg(shiftValue).arg(from).arg(to));
            });
            --j;
        }
        ...
        arr[finalPos] = key;  // Update local array
...
void SortingVisualizer::enqueueMergeSortRecursive(std::vector<int> &arr, int left, int right)
{
    ...
    while (i <= mid && j <= right) {
        // Show comparing state for the two elements being compared
        const int currentI = i;
        const int currentJ = j;
        const int valI = arr[i];
        const int valJ = arr[j];
        
        m_stepQueue.push([this, currentI, currentJ, valI, valJ]() {
            clearTransientStates();
            setStatesForIndices({currentI, currentJ}, ArrayVisualizer::BarState::Comparing);
            setStatusMessage(tr("Comparing %1 (index %2) and %3 (index %4)")
                                .arg(valI).arg(currentI).arg(valJ).arg(currentJ));
        });
